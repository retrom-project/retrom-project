...
核心输入变更须验证标准手柄方向移动和确认；取消可选，不得因缺少取消阻断接入。复核同一映射配置中的单按钮单目标，禁止原生按钮与键盘重复发送来补足确认或取消；真实键盘及已有可靠取消独立验证。具体 Case 使用当前 PFB Retrom 的 `docs/project-acceptance.md`，宿主界面的 B 返回仍按宿主 Case 验证。
