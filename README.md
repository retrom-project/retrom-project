...
Run `make pfb-list` to inspect every PFB development flow below `.worktree/`. The table includes its Retrom branch, creation time, effective status, PFB ID, and stable `http://<pfb-id>.localhost:3000` URL. If Retrom, runtime, any core or supporting repository in that PFB has tracked, untracked or submodule changes, the effective status is `DIRTY`; otherwise it is obtained from that PFB's read-only status command rather than inferred from the registry.
